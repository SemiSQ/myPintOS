...
/* Updates the priority of all the threads. */
void 
thread_update_priority (void)
{
  ...
  int f = 1<<14;
  int f2 = f/2;
  int load_avg1 = (59*f+f2)/60;
  int load_avg2 = (f+f2)/60;
  load_avg = (((int64_t) load_avg1) * load_avg) / f + load_avg2 * list_size (&ready_list);  
  int64_t recent_cpu1 = ((int64_t) load_avg) * 2 * f;
  int recent_cpu2 = load_avg * 2 + 1;
  recent_cpu1 = recent_cpu1 / recent_cpu2;
  load_avg = (load_avg + f2) / f;

  for (e = list_begin (&all_list); e != list_end (&all_list);
       e = list_next (e))
    {
      struct thread *t = list_entry (e, struct thread, allelem);
      t->recent_cpu = (t->recent_cpu * recent_cpu1 + f2)/f + t->nice;
      t->priority = thread_calculate_priority(t);
    }

  //thread_yield(); //this might be unnecessary
...
/* Calculates the current thread's priority by
   priority = PRI_MAX - (recent_cpu / 4) - (nice * 2). */
int
thread_calculate_priority (struct thread * t)
{
  int p = PRI_MAX - (t->recent_cpu / 4) - (t->nice * 2);
  return p;
}
...
thread_set_nice (int NICE) 
{
  ...
  int p = thread_calculate_priority (thread_current ());
  thread_set_priority (p);
}
